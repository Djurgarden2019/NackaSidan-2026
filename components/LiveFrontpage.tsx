...
export default function LiveFrontpage({ items, fetchedAt }: { items: LiveNewsItem[]; fetchedAt: string }) {
  ...
  const sections = Array.from(new Set(selected.map((item) => item.section))).slice(0, 5);
  ...
  return (
    <section className="section live-frontpage" aria-label="Senaste nyheter">
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:'8px',margin:'14px 0 20px'}} aria-label="Ämnen i nyhetsradarn">
        <span style={{fontSize:'.72rem',fontWeight:800,textTransform:'uppercase',letterSpacing:'.08em',padding:'7px 10px',background:'#171717',color:'#fff'}}>Just nu</span>
        {sections.map((section) => (
          <span key={section} style={{fontSize:'.72rem',fontWeight:700,textTransform:'uppercase',letterSpacing:'.06em',padding:'6px 10px',border:'1px solid #cfc8bb'}}>{section}</span>
        ))}
      </div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',gap:'24px',alignItems:'center',marginTop:'22px',paddingTop:'16px',borderTop:'1px solid #d8d2c6',fontSize:'.76rem',color:'#69645c'}}>
        <p style={{margin:0,maxWidth:'760px'}}>Rubrikerna hämtas direkt från anslutna källor och länkar till originalpubliceringen. NackaSidan prioriterar flödet redaktionellt men skriver inte om rubrikerna. Ämnesraden visar vilka bevakningsområden som dominerar bland de högst prioriterade nyheterna.</p>
