...
type Snapshot = { id: string; savedAt: number; label?: string; notes: string; checklist: boolean[]; lastSection: string; focus: boolean; compact: boolean; startedAt: number | null };
...
export default function LiveSessionSnapshots117() {
  ...
  return <section className="live-focus-secondary" aria-label="Sparade arbetsrundor" style={{margin:'-8px 0 18px',border:'1px solid #d8d2c6',background:'#fff'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'10px',padding:'9px 10px',flexWrap:'wrap'}}><div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}><input value={label} onChange={(e)=>setLabel(e.target.value)} maxLength={40} placeholder="Namn på ögonblicksbild (valfritt)" aria-label="Namn på ögonblicksbild" style={{border:'1px solid #cfc8bb',padding:'7px 8px',fontSize:'.68rem',minWidth:'190px'}}/><button type="button" onClick={saveSnapshot} style={{border:'1px solid #171717',background:'#fff',color:'#171717',padding:'7px 10px',fontSize:'.68rem',fontWeight:800,cursor:'pointer'}}>{saved?'Ögonblicksbild sparad ✓':'Spara ögonblicksbild'}</button></div><button type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open} style={{border:0,background:'transparent',padding:'4px',fontSize:'.68rem',fontWeight:800,cursor:'pointer',color:'#5f5a52'}}>Historik ({snapshots.length}) {open?'−':'+'}</button></div>{open&&<div style={{borderTop:'1px solid #e4ded4',padding:'8px 10px'}}>{snapshots.length===0?<p style={{margin:0,fontSize:'.7rem',color:'#69645c'}}>Inga sparade ögonblicksbilder ännu.</p>:<div style={{display:'grid',gap:'6px'}}>{snapshots.map((snapshot)=><div key={snapshot.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'10px',padding:'7px 8px',background:'#faf7f1',fontSize:'.68rem'}}><div><div style={{fontWeight:800}}>{snapshot.label || 'Arbetsrunda'}</div><div style={{marginTop:'2px'}}>{new Date(snapshot.savedAt).toLocaleString('sv-SE',{dateStyle:'short',timeStyle:'short'})}</div><div style={{color:'#69645c',marginTop:'2px'}}>{snapshot.checklist.filter(Boolean).length}/4 klara · {snapshot.notes?'anteckningar':'inga anteckningar'}</div></div><div style={{display:'flex',gap:'6px'}}><button type="button" onClick={()=>restore(snapshot)} style={{border:'1px solid #b6afa3',background:restoring===snapshot.id?'#fff1f0':'#fff',padding:'6px 8px',fontSize:'.64rem',fontWeight:800,cursor:'pointer',color:restoring===snapshot.id?'#9f1d20':'#171717'}}>{restoring===snapshot.id?'Bekräfta återställning':'Återställ'}</button><button type="button" onClick={()=>remove(snapshot)} style={{border:'1px solid #d8d2c6',background:'#fff',padding:'6px 8px',fontSize:'.64rem',fontWeight:800,cursor:'pointer',color:deleting===snapshot.id?'#9f1d20':'#69645c'}}>{deleting===snapshot.id?'Bekräfta ta bort':'Ta bort'}</button></div></div>)}</div>}</div>}</section>;
}
