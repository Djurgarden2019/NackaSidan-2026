...
const descriptions: Record<string, string> = {
  'stockholm-latest': 'Senaste verifierade publicerade artiklarna från Stockholm.',
  'stockholm-traffic': 'Tunnelbana, pendeltåg, buss, väg, cykel och sjötrafik – med färskhetskontroll.',
  'stockholm-districts': 'Följ nyheter och beslut i din del av Stockholm.',
  'stockholm-housing': 'Planerade bostäder, detaljplaner, byggstarter och färdigställda projekt.',
  'stockholm-business': 'Företag, jobb, tech, fastigheter, handel och besöksnäring.',
  'stockholm-culture': 'Konserter, teater, konst, film, litteratur och evenemang.',
  'stockholm-safety': 'Trygghet, polis, räddningstjänst och rättsväsende med tydlig källhantering.'
};
...
export default function StockholmPage() {
  ...
  const modules = stockholmFront176.filter((module) => module.kind !== 'lead').sort((a, b) => a.priority - b.priority);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section id="senaste" className="grid gap-8 border-b border-neutral-300 py-8 lg:grid-cols-[2fr_1fr]">
          ...
          <aside className="border-t border-neutral-300 pt-5 lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0">
            ...
            <div className="mt-6 rounded-lg bg-neutral-100 p-5">
              ...
              <p className="mt-2 text-sm text-neutral-600">Live-status visas först efter aktuell kontroll mot auktoritativ källa.</p>
            </div>
          </aside>
        </section>
      )}
      ...
      <section className="grid gap-5 border-t border-neutral-300 py-9 md:grid-cols-2 lg:grid-cols-3">
        {modules.map((module) => (
          <div id={module.id} key={module.id} className="border-t-4 border-neutral-950 pt-3">
            <h2 className="text-xl font-black">{module.title}</h2>
            <p className="mt-2 text-sm leading-6 text-neutral-600">{descriptions[module.id] ?? 'Fördjupad lokal bevakning av Stockholm.'}</p>
          </div>
        ))}
      </section>
    </main>
