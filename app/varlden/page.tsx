...
const lenses=[
 {title:'Säkerhet',text:'Konflikter, försvar och diplomati – med fokus på vad som faktiskt förändrar riskbilden.',href:'/varlden#veckans-sammanhang'},
 {title:'Ekonomi & handel',text:'Energi, sjöfart, tullar och leveranskedjor binder samman händelser långt från Sverige.',href:'/varlden#veckans-sammanhang'},
 {title:'Demokrati & makt',text:'Val, institutioner och politiska vägval avgör hur länder kan agera på längre sikt.',href:'/varlden#veckans-sammanhang'}
];
...
export default function WorldPage(){return <main><div className="shell">
 ...
 <section className="section"><SectionIntro eyebrow="Tre perspektiv" title="Så läser vi världen" text="Vi försöker skilja det dramatiska från det betydelsefulla och koppla dagens händelser till utvecklingen på längre sikt."/><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:16,marginTop:20}}>{lenses.map((item,index)=><Link key={item.title} href={item.href} style={{borderTop:'3px solid #111',paddingTop:14}}><div className="meta">0{index+1} · Perspektiv</div><h3 style={{fontSize:24,marginTop:8}}>{item.title}</h3><p>{item.text}</p><span className="text-link">Läs sammanhanget →</span></Link>)}</div></section>
