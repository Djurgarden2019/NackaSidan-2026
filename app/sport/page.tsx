...
export default async function SportPage(){
 ...
 return <main><div className="shell sport-desk">
  <header className="sport-desk-head"><div><div className="kicker">Uppdaterad {updatedLabel(now)}</div><h1>Sport</h1><p className="lead">6–8 verifierade sportartiklar med sammanfattning, publiceringstid, källa och redaktionell prioritet. Ingenting äldre än 72 timmar visas.</p></div><nav aria-label="Sportområden"><a href="#huvudnyhet">Huvudnyhet</a><a href="#senaste">Senaste</a><a href="#resultat">Resultat</a><a href="#kalender">Kalender</a><Link href="/sport/champions-league">Champions League</Link></nav></header>

  {lead?<section className="sport-lead" id="huvudnyhet"><article><div className="kicker">Huvudnyhet · {lead.sport}</div><h2><Link href={lead.href}>{lead.title}</Link></h2><p className="lead">{lead.dek}</p><div className="sport-lead-facts"><p><strong>Källa:</strong> {lead.source}</p><p><strong>Publicerad:</strong> {lead.date}</p><p>{lead.fact}</p></div><Link className="button" href={lead.href}>{lead.internal?'Läs hela artikeln':'Öppna originalkällan'}</Link></article><aside><div className="kicker">Bevakningen just nu</div><strong>{articles.length}</strong><span>aktuella artiklar</span><hr/><p>Flödet kontrolleras varje timme. Endast verifierade poster inom 72 timmar visas.</p></aside></section>:<section className="sport-empty"><div className="kicker">Huvudnyhet</div><h2>Nästa verifierade sportartikel förbereds</h2><p>Flödet fylls på när anslutna källor publicerar nya uppgifter.</p></section>}
  ...
  <section className="sport-sources sport-panel" id="standard"><div className="kicker">Redaktionell standard</div><h2>Mer information i varje post</h2><div className="sport-standard-grid"><p><strong>Nyheten.</strong> Händelse, resultat och bekräftade uppgifter.</p><p><strong>Sammanhang.</strong> Tävling, prioritet och vad uppgiften gäller.</p><p><strong>Aktualitet.</strong> Exakt publiceringstid och högst 72 timmars ålder.</p><p><strong>Källan.</strong> Tydlig originalkälla och direktlänk.</p></div></section>
