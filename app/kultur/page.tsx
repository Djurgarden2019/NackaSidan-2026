...
export default async function CulturePage(){
 ...
 const latest=live.items.filter(item=>(item.section==='Kultur'||item.sourceSection==='Kultur')&&item.source.startsWith('SVT')).sort((a,b)=>Date.parse(b.published)-Date.parse(a.published)).slice(0,8);
 ...
 return <main><div className="shell culture-desk">
  ...
  {latest.length>0&&<section className="culture-shelf" id="nyheter"><div className="culture-shelf-head"><div><div className="kicker">Svenska kulturnyheter · Senaste 48 timmarna</div><h2>Dagens kultur</h2></div></div><div className="culture-shelf-grid">{latest.map((item,index)=><article key={item.link} className={index===0?'culture-item culture-item-lead':'culture-item'}>{item.image&&<a href={item.link} target="_blank" rel="noopener noreferrer" style={{display:'block',aspectRatio:'16/9',overflow:'hidden',marginBottom:14}}><img src={item.image} alt="" loading={index===0?'eager':'lazy'} style={{width:'100%',height:'100%',objectFit:'cover'}}/></a>}<div className="kicker">{item.source} · {timeLabel(item.published)}</div><h3><a href={item.link} target="_blank" rel="noopener noreferrer">{item.title}</a></h3>{item.summary&&<p>{item.summary}</p>}<a className="text-link" href={item.link} target="_blank" rel="noopener noreferrer">Läs hos källan →</a></article>)}</div></section>}
  <Shelf id="fordjupning" eyebrow="Redaktionell fördjupning" title="Kulturartiklar" items={cultureNews} imageFor={imageFor}/>
