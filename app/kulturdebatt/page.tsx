...
export default function CultureDebate(){return <main><div className="shell">
 ...
 <section className="section no-top"><SectionIntro eyebrow="Veckans idédebatt" title="Fyra frågor att följa" text="Vi följer inte bara utspelen, utan vad som händer när principerna blir regler, budgetar och institutionella beslut."/><div className="debate-grid">{debates.map(([section,title,text,href],index)=><Link className="debate-card" href={href} key={title}><div className="meta">0{index+1} · {section}</div><h2>{title}</h2><p>{text}</p><AnalysisBox>Det avgörande blir hur principerna översätts till regler, finansiering och praktiska beslut. Därför skiljer vi mellan den aktuella konflikten och den långsiktiga samhällseffekten.</AnalysisBox><span className="text-link">Läs mer →</span></Link>)}</div></section>
