...
export default async function WeekendArticlePage({ params }: PageProps) {
  ...
  return <main><div className="shell"><article className="article article-premium weekend-article">
    ...
    <div className="article-part-label">02 · Själva artikeln</div><div className="article-body">{article.news.map(section => <section className="article-section" key={section.heading}><h2>{section.heading}</h2>{section.paragraphs.map(paragraph => <p key={paragraph}>{paragraph}</p>)}</section>)}</div>
    ...
    <div className="article-part-label">04 · Källhänvisning</div><section className="sport-article-sources"><h2>Tydliga och klickbara källor</h2><ul>{article.sources.map(source => <li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer">{source.label} ↗</a></li>)}</ul></section>
