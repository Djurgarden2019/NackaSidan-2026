...
export default async function Page(){
 ...
 const economy=items.filter(item=>(item.section==='Ekonomi'||item.sourceSection==='Ekonomi')&&isSwedishEconomicSource(item.source)).sort((a,b)=>Date.parse(b.published)-Date.parse(a.published)).slice(0,20);
 const household=economy.filter(item=>/ränta|inflation|pris|hushåll|bostad|lön|jobb|arbetslös|el|krona/i.test(item.title)).slice(0,8);
 const companies=economy.filter(item=>/företag|bolag|börs|bank|export|industri|handel|vinst|konkurs/i.test(item.title)).slice(0,8);
 ...
 <header className="economy-head"><div><div className="kicker">Svenska ekonominyheter · Uppdaterad {today()}</div><h1>Ekonomi</h1><p className="lead">Dagens viktigaste ekonominyheter på svenska om räntor, inflation, hushåll, företag, börs och konjunktur.</p></div><nav aria-label="Ekonomisidans avdelningar"><a href="#senaste">Senaste</a><a href="#hushall">Hushåll</a><a href="#foretag">Företag</a><a href="#bors">Marknader</a></nav></header>
 ...
 {household.length>0&&<section className="economy-section" id="hushall"><div className="economy-title"><div><div className="kicker">Privatekonomi</div><h2>Räntor, priser och hushåll</h2></div></div><div className="economy-news-grid">{household.map((item,index)=><NewsCard item={item} lead={index===0} key={item.link}/>)}</div></section>}
 {companies.length>0&&<section className="economy-section" id="foretag"><div className="economy-title"><div><div className="kicker">Näringsliv</div><h2>Företag, arbetsmarknad och börs</h2></div></div><div className="economy-news-grid">{companies.map((item,index)=><NewsCard item={item} lead={index===0} key={item.link}/>)}</div></section>}
