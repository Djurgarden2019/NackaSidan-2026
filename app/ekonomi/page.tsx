...
const themes=[
 {title:'Räntan och bolånen',text:'Vi följer styrräntan, bankernas boräntor och hur förändringarna slår mot hushållens månadsbudget.'},
 {title:'Priser och köpkraft',text:'Inflation är mer än en procentsiffra. Mat, boende, energi och tjänster avgör hur mycket som faktiskt blir kvar.'},
 {title:'Jobb och konjunktur',text:'Arbetsmarknaden visar om återhämtningen når hushållen och vilka branscher som går först in i nästa fas.'},
 {title:'Börs och företag',text:'Bolagens investeringar, vinster och framtidstro ger tidiga signaler om vart ekonomin är på väg.'}
];
...
export default function Page(){return <main><div className="shell">
 ...
 <section className="section"><div className="kicker">Fyra frågor vi följer</div><h2>Det som formar din ekonomi</h2><div className="article-list">{themes.map((item,index)=><article className="article-row" key={item.title}><div className="meta">{String(index+1).padStart(2,'0')} · Ekonomi</div><div><h2>{item.title}</h2><p>{item.text}</p></div></article>)}</div></section>
