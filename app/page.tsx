...
export default async function Home(){
 ...
 return <main><div className="shell world-front">
  ...
  <section className="world-focus" aria-label="Bevakningsområden"><div><div className="kicker">Detta följer vi</div><h2>Fem frågor som kan flytta världsläget</h2></div><ol>{['Sveriges val och den kommande regeringsbildningen','Angreppen mot Ukrainas civila ekonomi','Hotet mot sjöfarten genom Hormuz och Röda havet','Säkerheten vid kärnkraftverket i Zaporizjzja','BRICS-ländernas försök till medling i Mellanöstern'].map((item,index)=><li key={item}><span>{String(index+1).padStart(2,'0')}</span>{item}</li>)}</ol></section>
