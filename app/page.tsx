...
export default async function Home(){
 ...
 return <main><div className="shell world-front">
  ...
  <section className="world-focus" aria-label="Bevakningsområden"><div><div className="kicker">Detta följer vi</div><h2>Fem frågor som kan flytta världsläget</h2></div><ol>{['Samtalen mellan USA och Iran och effekten på energipriserna','Xi Jinpings USA-besök och relationen mellan världens två största ekonomier','EU:s Rysslandssanktioner och stödet till Ukraina','Made in EU-reglernas följder för handel och svensk industri','Den svenska regeringsbildningen och budgetförhandlingarna efter valet'].map((item,index)=><li key={item}><span>{String(index+1).padStart(2,'0')}</span>{item}</li>)}</ol></section>
