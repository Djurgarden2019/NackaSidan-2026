...
export default async function Home(){
 ...
 return <main><div className="shell world-front">
  <section className="world-front-heading"><div><div className="kicker">Världen · Fredag 28 augusti 2026</div><h1>Världen just nu</h1><p className="lead">Krig, diplomati, ekonomi och globala maktförskjutningar – med verifierade källor och fördjupande analys.</p></div><Link className="text-link" href="/varlden">Öppna hela utrikesbevakningen →</Link></section>

  <section className="world-lead" aria-labelledby="world-lead-title"><div className="world-lead-media"><img src={lead.image} alt="Karta över Hormuzsundet och Persiska viken"/><span>Karta: Wikimedia Commons</span></div><article><div className="kicker">Huvudnyhet · {lead.section}</div><h2 id="world-lead-title"><a href={lead.href} target="_blank" rel="noreferrer">{lead.title}</a></h2><p className="lead">{lead.summary}</p><div className="world-lead-actions"><a className="button" href={lead.href} target="_blank" rel="noreferrer">Läs originalkällan</a><Link className="text-link" href="/analys/iran-kriget-dyrt-dodlage">Läs vår analys →</Link></div></article></section>

  <section className="world-grid-section" aria-labelledby="world-latest-title"><div className="world-section-head"><div><div className="kicker">Senaste utvecklingen</div><h2 id="world-latest-title">Nyhetsläget i världen</h2></div><span className="meta">Uppdateras var 15:e minut</span></div><div className="world-story-grid">{worldFeatures.slice(1).map((story,index)=><article className={index===0?'world-story world-story-major':'world-story'} key={story.title}><div className="kicker">{story.section}</div><h3><a href={story.href} target="_blank" rel="noreferrer">{story.title}</a></h3><p>{story.summary}</p><a className="text-link" href={story.href} target="_blank" rel="noreferrer">Läs hos källan →</a></article>)}</div></section>
