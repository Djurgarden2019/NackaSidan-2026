...
export default async function Home(){
 ...
 const freshNews=live.items.filter(item=>within72Hours(item.published)&&!isLocalNews(item));
 ...
 return <main><div className="shell world-front">
  ...
  <section className="world-live" aria-labelledby="world-live-title"><div className="world-section-head"><div><div className="kicker">Direkt från redaktionerna</div><h2 id="world-live-title">Senaste utrikesnyheterna</h2></div><span className="meta">Endast de senaste 72 timmarna</span></div>{worldLive.length?<div className="world-live-list">{worldLive.map((item,index)=><a href={item.link} target="_blank" rel="noreferrer" className="world-live-row" key={`${item.link}-${index}`}><time>{dateLabel(item.published)}</time><div><span className="kicker">{item.source}</span><h3>{item.title}</h3></div><span aria-hidden="true">→</span></a>)}</div>:<div className="world-empty"><h3>Liveflödet uppdateras</h3><p>Inga verifierade utrikesnyheter yngre än 24 timmar är tillgängliga just nu.</p></div>}</section>
