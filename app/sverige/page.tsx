...
export default async function SverigePage(){
 ...
 return <main className="standard-section-page">
  ...
  <section id="dagens-nyheter" className="border-b border-neutral-300 py-10">
   ...
   {dailyNews.length?<div className="mt-6 divide-y divide-neutral-300 border-t-4 border-neutral-950">{dailyNews.map((item,index)=><a key={`${item.link}-${index}`} href={item.link} target="_blank" rel="noreferrer" className="grid gap-2 py-5 hover:bg-neutral-50 sm:grid-cols-[9rem_1fr_auto] sm:items-start"><time className="text-sm text-neutral-500">{liveDate(item.published)}</time><div>{item.image&&<a href={item.link} target="_blank" rel="noreferrer" className="mb-4 block aspect-video overflow-hidden"><img src={item.image} alt="" loading="lazy" className="h-full w-full object-cover"/></a>}<p className="text-xs font-bold uppercase tracking-wide text-red-800">{item.source}</p><h3 className="mt-1 text-xl font-black leading-tight">{item.title}</h3>{item.summary&&<p className="mt-2 line-clamp-2 leading-7 text-neutral-600">{item.summary}</p>}</div><span className="font-bold" aria-hidden="true">→</span></a>)}</div>:<p className="mt-5 border border-neutral-200 bg-neutral-50 p-5 text-neutral-600">Inga nya Sverige-nyheter visas just nu.</p>}
  </section>
