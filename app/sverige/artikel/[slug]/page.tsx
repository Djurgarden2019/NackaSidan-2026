...
export default async function SwedenArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!article) notFound();
  const related=swedenArticles220.filter(item=>item.status==='published'&&item.slug!==article.slug&&(item.section===article.section||item.region===article.region)).slice(0,3);
  return <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
    ...
    <article className="mx-auto mt-8 max-w-4xl">
      ...
      <div className="mt-5 space-y-10">{article.body.slice(0,1).map((block, index) => <section key={index} className="max-w-3xl">{block.heading ? <h2 className="text-3xl font-black tracking-tight">{block.heading}</h2> : null}<div className="mt-4 space-y-5">{block.paragraphs.map((paragraph, paragraphIndex) => <p key={paragraphIndex} className="text-lg leading-8 text-neutral-800">{paragraph}</p>)}</div></section>)}</div>
      <div className="article-part-label">03 · Analys och konsekvenser</div>
      <div className="mt-5 space-y-10">{article.body.slice(1).map((block, index) => <section key={index} className="max-w-3xl">{block.heading ? <h2 className="text-3xl font-black tracking-tight">{block.heading}</h2> : null}<div className="mt-4 space-y-5">{block.paragraphs.map((paragraph, paragraphIndex) => <p key={paragraphIndex} className="text-lg leading-8 text-neutral-800">{paragraph}</p>)}</div></section>)}</div>
      <div className="article-part-label">04 · Källhänvisning</div>
      <section className="mt-5 pt-2"><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.18em] text-neutral-500">Transparens</p><h2 className="mt-1 text-2xl font-black">Klickbara källor</h2></div><span className="text-xs text-neutral-500">Öppnas i ny flik</span></div><ul className="mt-5 space-y-3">{article.sourceUrls.map((source,index) => <li key={source} className="border-t border-neutral-200 pt-3"><a className="break-all text-sm font-bold underline underline-offset-4" href={source} target="_blank" rel="noopener noreferrer">Källa {index+1}: {source} ↗</a></li>)}</ul></section>
      {related.length?<section className="mt-12 border-t py-7"><p className="text-xs font-black uppercase tracking-[.18em] text-neutral-500">Läs vidare</p><h2 className="mt-1 text-2xl font-black">Relaterat från Sverige</h2><div className="mt-5 grid gap-4 md:grid-cols-3">{related.map(item=><article key={item.slug} className="border-t-2 border-neutral-950 pt-3"><div className="text-[11px] font-black uppercase tracking-wide text-neutral-500">{item.region||item.section}</div><h3 className="mt-1 text-lg font-black leading-snug"><Link href={`/sverige/artikel/${item.slug}`} className="hover:underline">{item.title}</Link></h3><p className="mt-2 text-sm leading-6 text-neutral-600">{item.intro}</p></article>)}</div></section>:null}
