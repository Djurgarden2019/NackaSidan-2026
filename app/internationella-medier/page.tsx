...
const europe=['BBC Europe','Deutsche Welle','France 24 Europe','The Guardian World','Politico Europe','Sky News World'];
const usa=['New York Times World','NPR World','Washington Post World','CBS News World'];
const global=['Al Jazeera'];
...
function NewsList({title,items}:{title:string;items:Awaited<ReturnType<typeof getLiveNews>>['items']}){
 return <section className="border-b border-neutral-300 py-10"><p className="text-xs font-bold uppercase tracking-widest text-red-800">Senaste 72 timmarna</p><h2 className="mt-1 text-3xl font-black sm:text-4xl">{title}</h2>{items.length?<div className="mt-6 divide-y divide-neutral-300 border-t-4 border-neutral-950">{items.map((item,index)=><article key={`${item.link}-${index}`} className="grid gap-3 py-6 md:grid-cols-[10rem_1fr]"><div><time className="text-sm text-neutral-500">{timeLabel(item.published)}</time><p className="mt-1 text-xs font-bold uppercase tracking-wide text-red-800">{item.source}</p></div><div>{item.image&&<a href={item.link} target="_blank" rel="noreferrer" className="mb-4 block aspect-video overflow-hidden"><img src={item.image} alt="" loading="lazy" className="h-full w-full object-cover"/></a>}<h3 className="text-2xl font-black leading-tight"><a href={item.link} target="_blank" rel="noreferrer" className="hover:underline">{item.title}</a></h3>{item.summary&&<p className="mt-3 max-w-3xl leading-7 text-neutral-600">{item.summary}</p>}<a href={item.link} target="_blank" rel="noreferrer" className="mt-4 inline-block text-sm font-bold underline">Läs originalartikeln →</a></div></article>)}</div>:<p className="mt-5 border border-neutral-200 bg-neutral-50 p-5 text-neutral-600">Inga verifierade artiklar yngre än 72 timmar är tillgängliga från denna mediegrupp just nu.</p>}</section>
}
