...
export default async function DriftstatusPage() {
  ...
  return <main style={{ maxWidth:960, margin:'0 auto', padding:'72px 28px 100px' }}>
    <p style={{ color:'#a61919', fontWeight:800, letterSpacing:2, fontSize:13 }}>MAIN 348 · DRIFTSTATUS</p>
    <h1 style={{ fontFamily:'Georgia,serif', fontSize:'clamp(48px,7vw,82px)', lineHeight:.98, margin:'12px 0 18px' }}>Nyhetsradarns status</h1>
    <p style={{ fontFamily:'Georgia,serif', fontSize:20, lineHeight:1.45, maxWidth:760 }}>Visuell kontroll av live-data, statusregler, källhälsa och den deployment som faktiskt kör.</p>
    <nav aria-label="Driftverktyg" style={{ display:'flex', flexWrap:'wrap', gap:10, marginTop:24 }}>
      {[['/driftpanel','Öppna Driftpanelen'],['/api/news-health','Visa health-JSON'],['/api/news-health/live','Liveness-probe'],['/api/news-health/ready','Readiness-probe'],['/api/deployment','Deploy-proveniens API'],['/','Till startsidan']].map(([href,label]) => <Link key={href} href={href} style={{ border:'1px solid #111', padding:'10px 14px', textDecoration:'none', fontWeight:700 }}>{label}</Link>)}
    ...
    <section aria-label="Nästa operativa steg" style={{ marginTop:24, padding:'18px 20px', border:'3px solid #111', background: status === 'STABIL' && deployment.provenanceOk ? '#eef4ec' : '#fff4d8' }}>
      ...
      <div style={{ marginTop:14, fontSize:14, lineHeight:1.55 }}>Incidentordning: liveness → readiness → full health → deploy-proveniens → exakt Vercel-deployment → verifierad återställning.</div>
    </section>
    ...
    <section aria-label="Probe-status" style={{marginTop:24,padding:'16px 18px',background:'#f7f5f1',lineHeight:1.55}}><strong>Övervakningsprober:</strong> liveness svarar om appen kör. Readiness använder Nyhetsradarns hälsomodell. Deploy-proveniens kan kontrolleras maskinellt via GET eller HEAD på /api/deployment.</section>
