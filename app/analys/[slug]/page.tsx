...
export default async function AnalysisArticle({params}:{params:Promise<{slug:string}>}){
 ...
 return <main><div className="shell"><article className="article article-premium">
  ...
  <section style={{paddingTop:8,marginTop:8}}><h2>Klickbara källor</h2><ul>{item.sources.map(source=><li key={source.url}><a href={source.url} target="_blank" rel="noopener noreferrer">{source.label} ↗</a></li>)}</ul><p style={{fontSize:14,color:'#666'}}>Analysdelen är redaktionens tolkning av verifierade uppgifter. Osäkerheter redovisas separat.</p></section>
